...
constructor(options) {
    this._n_of_iterations = 0;
    this._rmseThreshold = 0.5;
    ...
    if (this._isMain){
        this._cameraController = new OrbitCameraController(this._camera, this._canvas, {"_gallery": this._gallery});
    } else {
...
setVolume(reader) {
    ...
    this._volume.readMetadata({
        onData: () => {
            this._volume.readModality(this._isMain, 'default', {
                onLoad: () => {
                    this._volume.setFilter(this._filter);
                    if (this._renderer) {
                        // Volume loaded
                        if (this._isMain){
                            this._gallery.setInitialTransferFunctions();
                        }
...
_calculateRMSE(){
    var arr = new Uint8Array(this._previousFrame.length);
    for (let i = 0; i < this._previousFrame.length; i++){
        arr[i] = Math.pow(this._previousFrame[i] - this._currentFrame[i], 2)
    }
...
_transformToGrayscale(frame){
    var out = new Uint8Array(this._canvas.width * this._canvas.height);
    for (let i = 0; i < frame.length - 2; i++){
        var r_val = frame[i];
        var g_val = frame[i + 1];
        var b_val = frame[i + 2];
        out[Math.floor(i / 3)] = (r_val + g_val + b_val) / 3;
    }
    return out;
}
...
_render() {
    ...
    if (!this._isMain){
        var frame = new Uint8Array(this._canvas.width * this._canvas.height * 3);
        gl.readPixels(0, 0, this._canvas.width, this._canvas.height, gl.RGB, gl.UNSIGNED_BYTE, frame);
        this._previousFrame = this._currentFrame;
        this._currentFrame = this._transformToGrayscale(frame);
    
        this._n_of_iterations += 1;
        if (this._n_of_iterations < 10){
            return
        }

        

        this._currentRMSEval = this._calculateRMSE();


        if (this._currentRMSEval < this._rmseThreshold){
            this.stopRendering();
            console.log("ending, n of iterations: ", this._n_of_iterations);
            console.log("rmse: ", this._currentRMSEval);
            return;
        }

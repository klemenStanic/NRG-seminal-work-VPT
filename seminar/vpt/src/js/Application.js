...
constructor() {
    ...
    this._gallery.getGalleryContainer().appendTo(document.body);
    
    
    // MainWindow
    this._renderingContext = new RenderingContext({"_isMain": true, "_gallery": this._gallery});
    this._canvas = this._renderingContext.getCanvas();
    this._canvas.className += 'renderer';
    this._renderSpace = new RenderSpace();
    this._renderSpace._element.append(this._canvas);
    
    this._renderSpace.appendTo(document.body);
...
_handleRendererChange(which) {
    ...
    this._gallery.chooseRenderer(which);
    ...
    if (which != "mip" && which != "iso" && which != "mcc"){
        this._rendererDialog = new dialogClass(renderer, {"_gallery": this._gallery});
        this._rendererDialog.appendTo(container);
        this._gallery._dialog = this._rendererDialog;
        this._gallery.show();
        this._gallery.setInitialTransferFunctions();
        if (this._isInitialization) {
            this._isInitialization = false;
        } else {
            this._gallery.resetGalleryItems();
        }
        
        
        this._renderSpace._element.style.width = "80%";
    } else {
        this._rendererDialog = new dialogClass(renderer);
        this._rendererDialog.appendTo(container);
        this._gallery.hide();
        this._renderSpace._element.style.width = "100%";
    }
}
...
_handleToneMapperChange(which) {
    ...
    this._gallery.chooseToneMapper(which);
...
_handleVolumeLoad(options) {
    if (options.type === 'file') {
        ...
        if (readerClass) {
            ...
            this._gallery._mainHistogram = this._renderingContext._volume._histogram;
            this._gallery.stopRendering();
            this._gallery.setVolume(reader);
...
_handleEnvmapLoad(options) {
    ...
    image.addEventListener('load', () => {
        ...
        this._gallery.setEnvironmentMapAndReset(image);
    });
